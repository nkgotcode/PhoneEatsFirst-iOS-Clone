...
public extension DocumentReference {
  ...
  func setData<T: Encodable>(from data: T,
                             encoder: Firestore.Encoder = Firestore.Encoder()) -> AnyPublisher<
    Void,
    Error
  > {
    Future<Void, Error> { [weak self] promise in
...
//  }
  ...
  func setData<T: Encodable>(from data: T, merge: Bool,
                             encoder: Firestore.Encoder = Firestore.Encoder()) -> AnyPublisher<
    Void,
    Error
  > {
...
//  }
  ...
  func cacheFirstGetDocument<D: Decodable>(
    as type: D.Type,
    documentSnapshotMapper: @escaping (DocumentSnapshot) throws -> D? = DocumentSnapshot
      .defaultMapper()
  ) -> AnyPublisher<D?, Error> {
