...
public extension Query {
  internal struct Publisher: Combine.Publisher {
    ...
    func receive<S>(subscriber: S) where S: Subscriber, Publisher.Failure == S.Failure,
  ...
  func publisher<D: Decodable>(
    ...
    documentSnapshotMapper: @escaping (DocumentSnapshot) throws -> D? = DocumentSnapshot
      .defaultMapper(),
    querySnapshotMapper: @escaping (QuerySnapshot, (DocumentSnapshot) throws -> D?)
      -> [D] = QuerySnapshot.defaultMapper()
...
extension QuerySnapshot {
  ...
  public static func defaultMapper<D: Decodable>()
    -> (QuerySnapshot, (DocumentSnapshot) throws -> D?) -> [D]
  {
